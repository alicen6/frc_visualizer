...
for entry in all_rows:
    ...
    red_value = 300 - (int(red_score) * 3)
    blue_value = 300 + (int(blue_score) * 3)

    draw = ImageDraw.Draw(image)
    draw.polygon([(first_x_point, 300), (second_x_point, red_value),
                  (third_x_point, 300)], fill=(red_red, blue_red, green_red))
    draw.polygon([(first_x_point, 300), (second_x_point, blue_value),
                  (third_x_point, 300)], fill=(red_blue, blue_blue, green_blue))
    first_x_point += 30
    second_x_point += 30
    third_x_point += 30
image.show()
image.save("2016week0.jpeg", "JPEG")
